  ...
  return PagedPlugin = (function(_super) {
    ...
    PagedPlugin.prototype.docpadReady = function(opts, next) {
      ...
      DocumentModel.prototype.getPagedUrl = function(pageNumber) {
        ...
        if (pageNumber === 0) {
          return firstPage.get('url').replace('index.html', '');
        }
        firstPageUrl = firstPage.get('firstPageUrl');
        if (firstPageUrl === '/') {
          newUrl = '/index-page-' + (pageNumber+1) + '.html';
        } else {
          newUrl = firstPageUrl.replace(/\.html/, '-page-' + (pageNumber+1) + '.html');
        }
    ...
    PagedPlugin.prototype.renderBefore = function(opts, next) {
      ...
      collection.forEach(function(document) {
        var meta = document.getMeta(),
           pagedCollectionName,
           categories;
         if (!meta.get('isFiltered')) {
           return;
         }

         pagedCollectionName = meta.get('pagedCollection') || meta.get('dataType') || meta.get('type');
         categories = docpad.getCollection(pagedCollectionName + "-categories");
         if (categories) {
           var base = document.toJSON();
           categories.forEach(function(cat) {
             cat = cat.attributes.cat;
             pagedDoc = docpad.createDocument(base);
             pagedDoc.set({
                newName: "category-" + cat,
                isGenerated: true
             });
             pagedDoc.setMeta({
                pagedCollection: pagedCollectionName + "," + cat
             });
             pagesToRender.add(pagedDoc);
             collection.push(pagedDoc);
           });
         }
      });
      ...
      pagesToRender.forEach(function(document) {
        tasks.addTask(function(complete) {
          docpad.log('info', 'Normalizing paging document ' + document.get('basename'));
        ...
        tasks.addTask(function(complete) {
          ...
          if(!document.get('isGenerated')) {
            basename = basename + '-page-' + (page.number + 1);
            document.id = document.id.replace(/\.html/, '-page-' + (page.number + 1) + '.html');
            document.set('basename', document.get('basename') + '-page-' + (page.number + 1));
          } else {
            basename = document.get('newName');
            document.set('basename', basename);
            document.set('In menu?', false);
        ...
        /*return tasks.addTask(function(complete) {
          ...
          if(typeof page !== 'undefined') {
            outFilename = outFilename.replace(basename, basename + '-page-' + (page.number + 1));
            outPath = outPath.replace(basename, basename + '-page-' + (page.number + 1));
            basename = basename + '-page-' + (page.number + 1);
          } else {
          }  
        /*
