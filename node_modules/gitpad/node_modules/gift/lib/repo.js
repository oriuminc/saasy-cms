(function() {
  ...
  module.exports = Repo = (function() {
    ...
    Repo.prototype.resetHEAD = function(step, hard, callback) {
      var options = {};
      if (hard) {
        options = {hard: true};
      }
      this.git("reset", options, "HEAD~"+step);
    }
    ...
    Repo.prototype.file_history = function(filename, limit, callback) {
      var _ref2;
      if (!callback) {
        _ref2 = [callback, limit], limit = _ref2[0], callback = _ref2[1];
      }
      if (!callback) throw new Error("a callback is required");
      if (!filename) throw new Error("a file name must be specified");
      if (!limit) limit = 10;
      Commit.file_history(this, filename, limit, callback);
    };
    ...
    Repo.prototype.duplicate_branch = function(newbranch, oldbranch, callback) {
      return this.git("checkout", {b: true}, [newbranch, oldbranch], callback);
    };
    ...
    Repo.prototype.delete_branch = function(name, hard, callback) {
      var _ref2, options;
      if (!callback) 
        _ref2 = [hard, callback], callback = _ref2[0], hard = _ref2[1];
      if (hard === true)
        options = {D: true};
      else 
        options = {d: true};
      return this.git("branch", options, name, function(err, stdout, stderr) {
        return callback(err);
      });
    };
    ...
    Repo.prototype.checkoutFile = function(commitID, filename, callback) {
      return this.git("checkout ", {}, commitID + " -- " + filename, callback);
    };
    ...
    Repo.prototype.cherrypick = function(commitID, options, callback) {
      var _ref2;
      if (!callback) {
        _ref2 = [callback, options], options = _ref2[0], callback = _ref2[1];
      }
      return this.git("cherry-pick", options, commitID, function(err, stdout, stderr) {
        return callback(err);
      })
    }

    Repo.prototype.merge = function(branch, options, msg, callback) {
      options = _.extend(options, {m : "'" + msg + "'"});
      return this.git("merge", options, branch, callback);
    };
