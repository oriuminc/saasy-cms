(function() {
  ...
  module.exports = Commit = (function() {
    ...
    Commit.file_history = function(repo, filename, limit, callback) {
      var _this = this;
      if (limit === 'all') {
        options = {"follow": true, "pretty": "raw"};

      } else {
        options = {"follow": true, "pretty": "raw", "max-count": limit};
      }
      
      return repo.git("log", options, filename, function(err, stdout, stderr) {
        if (err) return callback(err);
        return callback(null, _this.parse_commits(repo, stdout));
      })
    }
    ...
    Commit.parse_commits = function(repo, text) {
      var author, author_line, authored_date, commits, committed_date, committer, committer_line, encoding, id, lines, message_lines, parents, tree, _ref, _ref2;
      commits = [];
      lines = text.split("\n");
      while (lines.length) {
        ...
        if (!/^committer /.test(lines[0])) author_line.push(lines.shift());
        ...
        _ref2 = this.actor(committer_line), committer = _ref2[0], committed_date = _ref2[1];
